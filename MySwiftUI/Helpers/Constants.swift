//
//  Constants.swift
//  MySwiftUI
//
//  Created by Mario Andres Heberthardt Marchant on 18/8/25.
//
import SwiftUI

enum Constants {
    enum Layout {
        static let standardPadding: CGFloat = 16
        static let padding5: CGFloat = 5
        static let padding50: CGFloat = 50
        static let smallPadding: CGFloat = 8
        static let cornerRadius15: CGFloat = 15
        static let iconSize: CGFloat = 24
        static let offset50: CGFloat = 50
        static let offset30: CGFloat = 30
        static let minimumWidth150: CGFloat = 150
        static let minimumHeight350: CGFloat = 350
        static let minimumHeight200: CGFloat = 200
        static let spacing20: CGFloat = 20
    }
    
    enum Animation {
        static let standardDuration: Double = 0.3
        static let slowDuration: Double = 0.6
    }
    
    enum Timing {
        static let defaultTimerDuration: TimeInterval = 60
    }
    
    enum ColorOpacities {
        static let opacity1: Double = 0.1
        static let opacity2: Double = 0.2
        static let opacity3: Double = 0.3
        static let opacity4: Double = 0.4
        static let opacity5: Double = 0.5
        static let opacity6: Double = 0.6
        static let opacity7: Double = 0.7
        static let opacity8: Double = 0.8
        static let opacity9: Double = 0.9
    }
    
}
